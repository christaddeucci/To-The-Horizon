...
public class TerrainControllerSimple : MonoBehaviour {
    ...
    private void Update() {
        ...
        //varyingColor = new Color((1.0f % (playerTransform.position.x / 1000.0f)), (2.0f % (playerTransform.position.x / 1000.0f)), 0.25f, 1.0f);
        //Debug.Log(varyingColor.ToString());
    ...
    private Vector2 NoiseOffset(int xIndex, int yIndex) {
        Vector2 noiseOffset = new Vector2(
            (xIndex * noiseScale + startOffset.x) % 512,
            (yIndex * noiseScale + startOffset.y) % 512
        );
        if (noiseOffset.x < 0)
            noiseOffset = new Vector2(noiseOffset.x + 512, noiseOffset.y);
        if (noiseOffset.y < 0)
            noiseOffset = new Vector2(noiseOffset.x, noiseOffset.y + 512);
        return noiseOffset;
    }
